...
async def precess_payment(calback_query: CallbackQuery):
    price = [LabeledPrice(label='HP Victus', amount=70000)]
    
    await bot.send_invoice(
        chat_id=calback_query.from_user.id,
        title='HP Victus',
        payload='laptop',
...
async def successful_payment(message: Message):
    await message.reply('спасиобо за покупку!')
